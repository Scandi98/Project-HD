 ...
 * Custom font loading by Reich
 */
public final class JFont {
		 
		 public JFont(Component context, String name, int style, int size,
				   boolean smooth) throws FontFormatException, IOException {
					 this(null, context, new Font(name, style, size), smooth);;
		 }
		 
		 Client c = new Client();
		 
		 public JFont(String fontFileName, int size, boolean smooth) throws FontFormatException, IOException {
			 	float amount = Float.parseFloat(Integer.toString(size));
				font = Font.createFont(Font.TRUETYPE_FONT, new File(signlink.findcachedir() + fontFileName));
				font = font.deriveFont(amount);
				fontMetrics = c.getGameComponent().getFontMetrics(font);
				if (fontMetrics == null)
					throw new RuntimeException();

				ascent = fontMetrics.getAscent();
				Rectangle bounds = font.getMaxCharBounds(
						fontMetrics.getFontRenderContext()).getBounds();
				...
				if (smooth)
					bufferGraphics.setRenderingHint(
							...
							smooth ? RenderingHints.VALUE_TEXT_ANTIALIAS_ON
									: RenderingHints.VALUE_TEXT_ANTIALIAS_OFF);

				bufferGraphics.setColor(new Color(FOREGROUND, true));
				charCache = new JFontChar[256][];
			}

	public JFont(String fontFileName, Component context, Font font, boolean smooth) throws FontFormatException, IOException {
		if (context == null || font == null)
			throw new IllegalArgumentException();
		this.font = font = Font.createFont(Font.TRUETYPE_FONT, new File(signlink.findcachedir() + fontFileName));
	...
	public JFont(URL url) throws IOException {
		this(url.openStream());
	}

	public JFont(String file) throws IOException {
		this(new File(file));
	}

	public JFont(File file) throws IOException {
		this.font = null;
	...
	public JFont(byte[] data) throws IOException {
		this(new ByteArrayInputStream(data, 0, data.length));
	}

	public JFont(byte[] data, int offset, int length) throws IOException {
		this(new ByteArrayInputStream(data, offset, length));
	}

	public JFont(InputStream in) throws IOException {
		this.font = null;
		load(in);
	}
	...
	private void load(InputStream in0) throws IOException {
		...
		for (int i = 0; i != 0x10000; ++i)
			provideFontChar((char) i, new JFontChar(in));
	...
	public void writeTo(OutputStream out0) throws IOException {
		...
		writeInt(out, ascent);
		for (int i = 0; i != 0x10000; ++i) {
			JFontChar chr = getFontChar((char) i, false);
			if (chr == null) {
				out.writeByte(0);
				continue;
			}
	...
	public int drawChar(char chr, int x, int y, int color, boolean validAlpha) {
		JFontChar fontChar = getFontChar(chr);
		drawPixels(fontChar.pixels, fontChar.width, fontChar.height,
				fontChar.xOffset + x, fontChar.yOffset + y - ascent,
				validAlpha ? color : color | 0xff000000);
	...
	public Rectangle stringBounds(String str) {
		return new Rectangle(stringX(str), stringY(str), stringWidth(str),
				stringHeight(str));
	}
	...
	public void provideFontChar(char chr, JFontChar fontChar) {
		if (disposed) {
			return;
		}
		...
		int sector = chr >>> 8;
		int index = chr & 0xff;
		JFontChar[][] charCache = this.charCache;
		JFontChar[] cacheSector = charCache[sector];
		if (cacheSector != null) {
			cacheSector[index] = fontChar;
			return;
		}
		cacheSector = charCache[sector] = new JFontChar[256];
		cacheSector[index] = fontChar;
	}

	public JFontChar getFontChar(char chr) {
		return getFontChar(chr, true);
	}

	public JFontChar getFontChar(char chr, boolean giveDummy) {
		if (disposed)
		...
		JFontChar[] cacheSector = charCache[sector];
		if (cacheSector != null) {
			JFontChar fontChar = cacheSector[index];
			if (fontChar != null)
				return fontChar;

		}
		JFontChar fontChar = getFontChar0(chr);
		if (fontChar == null) {
			if (!giveDummy)
				return null;
	...
	private static BufferedImage wrapPixels(int[] pixels, int width, int height) {
		DirectColorModel model = new DirectColorModel(32, 0xff0000, 0xff00,
				0xff, 0xff000000);
		return new BufferedImage(model, Raster.createWritableRaster(
				model.createCompatibleSampleModel(width, height),
				new DataBufferInt(pixels, width * height), null), false,
	...
	private void drawPixels(final byte[] pixels, final int width,
			final int height, int xPos, int yPos, final int color) {
		if (width < 1 || height < 1)
			return;
	...
	private JFontChar getFontChar0(char chr) {
		...
		bufferGraphics.drawString(String.valueOf(chr), 0,
		...
		for (int i = 0; i != height; ++i) {
			for (int i1 = 0; i1 != width; ++i1)
				fontPixels[localIndex + i1] = (byte) (bufferPixels[globalIndex
						+ i1] >>> 24);
		...
		return new JFontChar(fontPixels, width, height, x, y,
				fontMetrics.charWidth(chr), bufferHeight);
	}
	...
	private static int blend(int dst, int src, int alpha) {
		...
		int delta = 255 - alpha;
		return (src & 0xff000000 | ((src & 0xff00ff) * alpha + (dst & 0xff00ff)
				* delta & 0xff00ff00 | (src & 0xff00) * alpha + (dst & 0xff00)
				* delta & 0xff0000) >>> 8);
	}
	...
	private static int readInt(DataInputStream in) throws IOException {
		...
		return b | ((in.readByte() & 0xff) << 7)
				| ((in.readByte() & 0xff) << 15)
				| ((in.readByte() & 0xff) << 23);
	}

	private static void writeInt(DataOutputStream out, int val)
			throws IOException {
		if ((val & 0x7f) == val) {
	...
	public static class JFontChar {
		public JFontChar(byte[] pixels, int width, int height, int xOffset,
				int yOffset, int widthOffset, int heightOffset) {
		...
		public JFontChar(InputStream in0) throws IOException {
			DataInputStream in = in0 instanceof DataInputStream ? (DataInputStream) in0
					: new DataInputStream(in0);
	...
	private static final JFontChar DUMMY_FONT_CHAR = new JFontChar(new byte[0],
			0, 0, 0, 0, 0, 0);
	private static final int FOREGROUND = 0xffffffff;
	private static final int BACKGROUND = 0x00000000;
	private boolean disposed;
	private JFontChar[][] charCache;
