...
public class ClanManager {
	...
	public int getTotalClans() {
		File localFile = new File(Config.LOAD_DIRECTORY + "clan/");
	...
	public void delete(Clan paramClan) {
		...
		File localFile = new File(Config.LOAD_DIRECTORY + "clan/" + paramClan.getFounder()
				+ ".cla");
		if (localFile.delete()) {
			Client localClient = (Client) PlayerHandler.getPlayer(paramClan
					.getFounder());
			if (localClient != null) {
				localClient.sendMessage("Your clan has been removed.");
			}
	...
	private Clan read(String paramString) {
		File localFile = new File(Config.LOAD_DIRECTORY + "clan/" + paramString + ".cla");
		if (!localFile.exists()) {
			return null;
		}
		try {
