...
//	private static void cookFish(Client c, int itemID, int xpRecieved,
	...
	public static void cookThisFood(Client p, int i, int object) {
		switch (i) {
		case 3142:
			cookFish(p, i, 100, 30, 3148, 3144, object);
			break;
		case 317:
		...
		case 389:
			...
			break;
		case 15270:
			cookFish(p, i, 216, 93, i + 4, i + 2, object);
			break;
		case 7944:
	...
	private static void cookFish(Client c, int itemID, int xpRecieved,
		...
		if (c.playerEquipment[c.playerHands] == 775) {
			chance = (int) (c.playerLevel[7] * 1.35) + 8;
		}
		if (chance <= 0) {
	...
	private static void cookTheFish(final Client c) {
		...
		if (c.playerSkillProp[7][5] > 0) {
			c.startAnimation(c.playerSkillProp[7][5] == 25465 ? 897 : 897);
		}
		...
		CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				if (c.playerSkillProp[7][5] > 0) {
					c.startAnimation(c.playerSkillProp[7][5] == 25465 ? 897
							: 897);
				}
				if (!c.stopPlayerSkill) {
					container.stop();
