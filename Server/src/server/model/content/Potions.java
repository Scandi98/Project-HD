...
public class Potions {
	...
	public void handlePotion(int itemId, int slot) {
		...
		if (System.currentTimeMillis() - c.potDelay >= 1200) {
			...
			case 3046:
				...
				break;
			case 12695:
				drinkSuperCombat(itemId, 12697, slot);
				break;
			case 12697:
				drinkSuperCombat(itemId, 12699, slot);
				break;
			case 12699:
				drinkSuperCombat(itemId, 12701, slot);
				break;
			case 12701:
				drinkSuperCombat(itemId, 229, slot);
				break;
			case 2436:
			...
			case 12905:
				drinkAntiVenom(itemId, 12907, slot);
				break;
			case 12907:
				drinkAntiVenom(itemId, 12909, slot);
				break;
			case 12909:
				drinkAntiVenom(itemId, 12911, slot);
				break;
			case 12911:
				drinkAntiVenom(itemId, 229, slot);
				break;
				
			case 12913:
				drinkAntiVenom(itemId, 12915, slot);
				break;
			case 12915:
				drinkAntiVenom(itemId, 12917, slot);
				break;
			case 12917:
				drinkAntiVenom(itemId, 12919, slot);
				break;
			case 12919://done does my function look right
				drinkAntiVenom(itemId, 229, slot);
				break;
			}
		}
	...
	private void drinkAntiVenom(int itemId, int i, int slot) {
		c.startAnimation(829);
		c.playerItems[slot] = i + 1;
		c.getItems().resetItems(3214);
		
		RSItemDefinition def = RSItemDefinition.valueOf(itemId);
		
		cureVenom(def.name.contains("+"));
	}
	...
	public void cureVenom(boolean immunity) {
		c.venomDamage = 6;// look at code
		c.venomDebuff = false;
		if (immunity) {
			c.lastVenomSip = System.currentTimeMillis();
		}
	}
	...
	public void drinkSuperCombat(int itemId, int replaceItem, int slot) {
		c.startAnimation(829);
		c.playerItems[slot] = replaceItem + 1;
		c.getItems().resetItems(3214);
		enchanceStat(0, true);
