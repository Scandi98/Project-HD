		...
		if (saradomin.size() > 0) {
			for (int player : saradomin) {
				if (PlayerHandler.players[player] != null) {
					...
					c.getPA().sendNewString(saradominScore + " = Saradomin",
							11148);
					c.getPA().sendNewString("Zamorak = " + zamorakScore, 11147);
					c.getPA().sendNewString(gameStartTimer * 3 + " secs", 11155);
					c.getPA().sendNewString("", 11158);
					c.getPA().sendNewString("", 11160);
					c.getPA().sendNewString("", 11162);
					c.getPA().sendNewString("", 11164);
					c.getPA().sendNewString("", 11168);
					if (saraFlagSafe) {
						c.getPA().sendNewString("@gre@Safe", 11166);
					} else {
						c.getPA().sendNewString("@red@Taken", 11166);
					}
					if (zammyFlagSafe) {
						c.getPA().sendNewString("@gre@Safe", 11167);
					} else {
						c.getPA().sendNewString("@red@Taken", 11167);
					}
					// update interface here
				} else {
