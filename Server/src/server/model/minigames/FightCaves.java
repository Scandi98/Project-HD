...
public class FightCaves {
	...
	public static final int TZ_KIH = 2189, TZ_KEK_SPAWN = 2191, TZ_KEK = 2192, TOK_XIL = 2193, YT_MEJKOT = 3124, KET_ZEK = 3125, TZTOK_JAD = 3127;
	...
	public void spawnNextWave(Client c) {
		if (c != null) {
			...
			if (c.tzhaarToKill != c.tzhaarKilled) {
				return;
			}
			int npcAmount = WAVES[c.waveId].length;
			c.sendMessage("Starting Wave "+c.waveId);
	...
	public static void tzKihEffect(Client c,int i, int damage) {
		if(NPCHandler.npcs[i].npcId == TZ_KIH) {
			if(c != null) {
	...
	public static void tzKekEffect(Client c, int i) {
		if(NPCHandler.npcs[i].npcId == TZ_KEK) {
