...
public class PestControl {
	 ...
	 */
	private static final int WAIT_TIMER = 40;
	/**
	 ...
	 */
	public static int[] portalHealth = { 300, 300, 300, 300 };
	public static int[] portals = { 1747, 1748, 1749, 1750 };
	/**
	...
	private int[][] pcNPCData = { { 1747, 2628, 2591 }, // portal
			{ 1748, 2680, 2588 }, // portal
			{ 1749, 2669, 2570 }, // portal
			{ 1750, 2645, 2569 }, // portal
			{ 2950, 2656, 2592 }, // knight
	};
	...
	public void process() {
		try {
			...
			if (waitTimer > 0)
				waitTimer--;
			else if (waitTimer == 0 && playersInGame() < 1)
				startGame();
			if (KNIGHTS_HEALTH == 0)
				endGame(false);
						...
						c.getPA()
								.sendNewString(
										"(Need " + PLAYERS_REQUIRED
												+ " to 25 players)", 21122);
						c.getPA().sendNewString("Points: " + c.pcPoints + "",
								21123);
						switch (waitTimer) {
						case 60:
		...
		for (Client player : gamePlayers.keySet()) {
			if (player != null) {
				for (int i = 0; i < portalHealth.length; i++) {
					if (portalHealth[i] > 0) {
						player.getPA().sendNewString("" + portalHealth[i] + "",
								21111 + i);
					} else
						player.getPA().sendNewString("Dead", 21111 + i);

				}
				player.getPA().sendNewString("" + KNIGHTS_HEALTH, 21115);
				player.getPA().sendNewString("" + player.pcDamage, 21116);
				if (gameTimer > 60) {
	...
	public static void setGameInterface2() {
		for (Client player : gamePlayers.keySet()) {
			if (player != null) {
				for (int i = 0; i < portalHealth.length; i++) {
					player.getPA().sendNewString(" ", 21111 + i);
				}
				player.getPA().sendNewString(" ", 21115);
				player.getPA().sendNewString(" ", 21116);
				player.getPA().sendNewString(" ", 21117);
			}
			player.getPA().sendNewString(
		...
		for (int[] aPcNPCData : pcNPCData) {
			for (int j = 0; j < NPCHandler.npcs.length; j++) {
				if (NPCHandler.npcs[j] != null) {
					if (NPCHandler.npcs[j].npcId == aPcNPCData[0])
	...
	public boolean allPortalsDead3() {
		...
		for (int j = 0; j < NPCHandler.npcs.length; j++) {
			if (NPCHandler.npcs[j] != null) {
				if (NPCHandler.npcs[j].npcId >= 1747
						&& NPCHandler.npcs[j].npcId <= 1750)
	...
	public static void leaveWaitingBoat(Client c) {
		if (waitingBoat.containsKey(c)) {
			...
			c.getPA().sendNewString(" ", 21120);
			c.getPA().sendNewString(" ", 21121);
			c.getPA().sendNewString(" ", 21122);
			c.getPA().sendNewString(" ", 21123);
	...
	public static void addToWaitRoom(Client player) {
		if (player != null) {
			...
			//player.sendMessage("Pest control is currently being re-worked.");
		}
	}
	...
	private void spawnNPC() {
		for (int[] aPcNPCData : pcNPCData) {
			Server.npcHandler.spawnNpc2(aPcNPCData[0], aPcNPCData[1], aPcNPCData[2],
